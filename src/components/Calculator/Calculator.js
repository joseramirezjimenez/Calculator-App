...
class Calculator extends Component {
    ...
    onButtonPress = (event) => {
        ...
        if (pressedButtonValue === 'C') {
            ...
            /* If the user had pressed a button with a value between 0 and 9 or the period, add that value to the equation */
            equation += pressedButtonValue;
        } else if (['+', '-', '*', '/', '%'].indexOf(pressedButtonValue) !== -1 ) {
            /* If the user has pressed a button with the value of an operator, add the operator to the equation with a space in either side */
            equation += ' ' + pressedButtonValue + ' ';
        } else if (pressedButtonValue === '=') {
            /* If the user hit the enter button, calculate stuff */
            ...
            /* If the user had pressed the back arrow, remove the last character from the equation string */
